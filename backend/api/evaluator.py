...
from concurrent.futures import (
    ThreadPoolExecutor,
)  # TODO: maybe import it inside the if statement


def evaluate_single(
    candidate: CandidateProfile, job_text: str, scorer
) -> Dict[str, Any]:
...
def evaluate_candidates(
    candidates: List[CandidateProfile], job_text: str, scorer: ScoringStrategy
) -> List[Dict[str, Any]]:
    if len(candidates) > 50:
        with ThreadPoolExecutor() as executor:
            results = list(
                executor.map(lambda c: evaluate_single(c, job_text, scorer), candidates)
            )
    else:
